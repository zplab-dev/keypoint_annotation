...
def train_model(slope, max_val, downscale=1, image_shape=(960,96), mask_error=False, total_epoch_num=25):
    ##Load in Data
    ...
    if os_type == 'Darwin':
        ...
        save_dir = '/mnt/lugia_array/Laird_Nicolette/deep_learning/keypoint_detection/new_api/production_dataloader_test/new_kp_maps/sigmoid_kp/'+str(total_epoch_num)+'_epochs/'+project_name
    ...
    fn.write('slope: {}\t, max_val: {}\t, image_shape: {}\t, downscale: {}\t\n'.format(slope, max_val, image_shape, downscale))
...
if __name__ == "__main__":
    try:
        slope, max_val = sys.argv[1], sys.argv[2]
    except IndexError:
        print("Please include slope and max_val")
        sys.exit(1)

    train_model(float(slope), float(max_val))
