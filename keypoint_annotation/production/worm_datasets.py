...
class WormKeypointDataset:
    ...
    def __call__(self, timepoint):
        ...
        #get keypoint maps
        keypoint_maps = self.generate_keypoint_maps(timepoint, worm_frame_image.shape)
        #scale keypoint maps
...
class VulvaClassifier:
    ...
    def __call__(self, timepoint):
        worm_frame_image = self.worm_frame_image(timepoint)
        vulva_class = self.get_vulva_class(timepoint)
...
class WormKeypointDatasetNoFlip:
    """Basically the same as the WormKeypointDataset above, but doesn't flip the wormframe image based on vulval side
